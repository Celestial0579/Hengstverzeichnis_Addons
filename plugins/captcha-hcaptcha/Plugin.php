<?php
// captcha-hcaptcha/Plugin.php
//
// Addon für Hengstverzeichnis_Framework, Teil von
// Celestial0579/Hengstverzeichnis_Addons#133: hCaptcha als wählbarer Anbieter
// für die drei captcha.*-Hooks des Kerns.
//
...
// Es sagt dem Betreiber an drei Stellen, was er damit tut: im Anzeigenamen
// der Anbieterauswahl, im Datenschutz-Kasten der Verwaltungsseite und in
// einem Hinweis unter dem Widget, den der BESUCHER sieht.
//
// WAS hCAPTCHA VON TURNSTILE UNTERSCHEIDET - und was nicht.
// hCaptcha gilt als der etablierte Ersatz für Google reCAPTCHA und bietet
// eine Auftragsverarbeitung mit EU-Bezug an. Das ändert die Rechtslage in
// EINEM Punkt: Der Betreiber kann einen Vertrag schliessen, der die
// Verarbeitung ordnet. Es ändert sie NICHT darin, dass die IP-Adresse des
// Besuchers das Haus verlässt und der Anbieter eine US-Gesellschaft ist
// (Intuition Machines, Inc.). Wer die Übermittlung ganz vermeiden will,
// nimmt captcha-altcha; wer sie eingehen will, braucht Rechtsgrundlage,
// Datenschutzerklärung und - bei EU-Verarbeitung - den passenden Vertrag.
//
// Anders als Turnstile zeigt hCaptcha den Besuchern in einem Teil der Fälle
// eine Bilderaufgabe. Das ist eine Zugänglichkeitsfrage: Nicht jeder kann
// Bilder zuordnen. hCaptcha bringt dafür eine eigene Barrierefreiheits-Lösung
// mit (accessibility.hcaptcha.com), die der Betreiber kennen sollte - siehe
// README.
//
...
// Wahl. Die Verwaltungsseite sagt das. Seit Framework#351 lässt sich der
// Anbieter je Formular wählen; ein Betreiber kann hCaptcha also für die
// Kontakt- und Deckanfragen nutzen und das DSGVO-Portal davon ausnehmen.
//
...
// Oberfläche dafür. Wer hier etwas ändert, sieht in captcha-turnstile und
// captcha-altcha nach, ob es dort genauso gilt.
//
// Installation (lokal im Framework-Repo):
//   cp -r captcha-hcaptcha plugins/captcha-hcaptcha
// Danach unter Admin -> Plugins verwalten (/admin/plugins) aktivieren,
// unter Admin -> hCaptcha-Schlüssel (Kachel im Dashboard) Sitekey und Secret
// hinterlegen und erst DANACH unter Admin -> Systemeinstellungen als
// Spam-Schutz auswählen. Vorher erscheint der Anbieter dort gar nicht.

namespace Plugin\CaptchaHcaptcha;
...
class Plugin {
     ...
     */
    public const SLUG = 'captcha-hcaptcha';
     ...
     *
     * Es sind VIER, nicht eine - das ist der Punkt, an dem hCaptcha sich von
     * Turnstile unterscheidet und an dem eine unvollständige Freigabe
     * besonders unangenehm ist: Mit nur `js.hcaptcha.com` lädt das Skript,
     * und danach bleibt der Rahmen leer. Der Fehler sieht dann aus wie ein
     * kaputtes Addon, nicht wie eine Policy.
     *
     *   js.hcaptcha.com         das Widget-Skript          (script-src)
     *   newassets.hcaptcha.com  Rahmen und Aufgabeninhalte (frame-src, script-src)
     *   assets.hcaptcha.com     ältere Auslieferung        (frame-src, script-src)
     *   api.hcaptcha.com        Rückrufe aus dem Rahmen    (connect-src)
     */
    public const CSP_ORIGINS = [
    ...
    public function permissions(): array {
        return [[
            ...
            'module_label' => 'Spam-Schutz: hCaptcha',
        ]];
    }
     ...
     *
     * Ohne Sitekey und Secret verweigerte jede Prüfung zuverlässig ihren
    ...
    public function anbieterMelden(array $providers): array {
        ...
        $providers[self::ANBIETER] = 'hCaptcha (Drittanbieter: Intuition Machines, Inc., USA)';
        return $providers;
    }
    ...
    public function widgetRendern(string $html, string $anbieter, string $kontext): string {
        ...
        if ($siteKey === '') {
            // Gewählt, aber nichts zu rendern: kein Fragment zurückgeben
            // heisst, der Kern rendert seine eigene Aufgabe (siehe
            // Captcha::renderField()). Das Formular bleibt benutzbar.
            return $html;
        }
     ...
     * dann, wenn wir wirklich eines haben.
     *
    ...
    public function antwortPruefen(?string $urteil, string $anbieter, string $kontext, array $input): ?string {
        ...
        if ($token === '' || strlen($token) > self::TOKEN_MAXLAENGE) {
            // Niemand hat das Widget gelöst (oder jemand schickt Müll). Dieses
            // Urteil können wir ohne Netzaufruf fällen - und wir fällen es,
            // statt hCaptcha mit leeren Anfragen zu belasten.
    ...
    /** hCaptcha-Tokens sind lang (P1_...), aber nicht beliebig lang. */
    private const TOKEN_MAXLAENGE = 8192;
 ...
 * aufzählen und löschen wie Tabellen.
 *
 ...
 * SMTP-Passwort und die TOTP-Secrets nimmt.
 *
 * OB ES JE IN EINER ANTWORT AUFTAUCHEN KANN: nein.
...
final class Konfiguration {
    ...
    public const SCHLUESSEL_SITE = 'plugin_captcha_hcaptcha_site_key';
    public const SCHLUESSEL_SECRET = 'plugin_captcha_hcaptcha_secret';
    public const SCHLUESSEL_FEHLER = 'plugin_captcha_hcaptcha_letzter_fehler';

    /**
     * hCaptcha-Sitekeys sind UUIDs, Secrets Hex-Zeichenketten mit Präfix -
     * beides deckt dieses bewusst weite Muster ab. Es prüft die FORM, nicht
     * die Gültigkeit; ob der Schlüssel stimmt, sagt erst der Anbieter, und
     * genau dafür gibt es den Fehlerkasten auf der Verwaltungsseite.
     */
    ...
    /**
     * Erst mit Sitekey UND Secret ist der Anbieter benutzbar - siehe
     ...
     * zeigen kann. Ein falsches Secret äussert sich für den Betreiber sonst
     * nur als "das Formular geht nicht mehr" - ohne Hinweis, wo er suchen
     * soll. Geschrieben wird nur bei Konfigurations- und Netzfehlern, nicht
     * bei jeder nicht bestandenen Prüfung.
     */
    public static function fehlerMerken(string $text): void {
        try {
            self::setzen(self::SCHLUESSEL_FEHLER, date('Y-m-d H:i') . ' - ' . $text);
        } catch (\Throwable $e) {
            // Diagnose-Komfort, kein Betriebszweck.
        }
    }
 ...
 * Sitekey.
 *
 * Das Anbieterskript steht IM FRAGMENT und nicht im Layout: renderField()
 * läuft nur dort, wo tatsächlich ein geschütztes Formular steht. Ein
 * Drittskript auf jeder Seite des Verzeichnisses wäre etwas anderes als
 * eines auf den geschützten Formularen.
 */
final class Widget {

    /** Feldname, unter dem hCaptcha seine Antwort in das Formular schreibt. */
    public const FELD = 'h-captcha-response';

    private const SKRIPT = 'https://js.hcaptcha.com/1/api.js';

    private function __construct() {}

    public static function fragment(string $siteKey, string $kontext): string {
        $id = 'hv-hcaptcha-' . preg_replace('/[^a-z0-9_-]/', '', strtolower($kontext));
        $idAttr = htmlspecialchars($id, ENT_QUOTES, 'UTF-8');

        return '<div class="form-group">'
            . '<label>Sicherheitsprüfung</label>'
            . '<div id="' . $idAttr . '" class="h-captcha"'
            ...
            . 'Geschützt durch hCaptcha. Dabei werden Ihre IP-Adresse und technische Angaben Ihres Browsers '
            . 'an Intuition Machines, Inc. (USA) übermittelt - '
            . '<a href="https://www.hcaptcha.com/privacy" target="_blank" rel="noopener noreferrer">Datenschutzerklärung von hCaptcha</a>. '
            . 'Wer die Bildaufgaben nicht lösen kann, findet unter '
            . '<a href="https://accessibility.hcaptcha.com/" target="_blank" rel="noopener noreferrer">accessibility.hcaptcha.com</a> '
...
/**
 * Der serverseitige Prüfaufruf gegen hCaptcha.
 *
 * FAIL-CLOSED BEI NETZFEHLERN. Antwortet der Anbieter nicht, ist das Ergebnis
 ...
 *
 * KEIN `remoteip`. hCaptcha erlaubt, die IP des Besuchers mitzuschicken; wir
 * tun es nicht. Der Anbieter sieht sie ohnehin, weil das Widget aus dem
...
final class Anbieter {
    ...
    private const URL = 'https://api.hcaptcha.com/siteverify';
     ...
     */
    private const ABGELAUFEN = [
        'invalid-or-already-seen-response',
        'expired-or-already-used-captcha',
    ];
     ...
     *
     * `sitekey-secret-mismatch` verdient dabei besondere Aufmerksamkeit - es
     * ist der Fehler, den man beim Umzug zwischen zwei hCaptcha-Konten baut,
     * und der einzige, dem man ohne diese Meldung nicht auf die Spur kommt.
     */
    ...
    public static function pruefen(string $secret, string $token, string $kontext): string {
        ...
        if ($antwort === null) {
            Konfiguration::fehlerMerken('hCaptcha war nicht erreichbar - Eingaben wurden abgewiesen (fail-closed).');
            PluginAudit::log(
                Plugin::SLUG,
                'Prüfung beim Anbieter fehlgeschlagen',
                'Formular ' . $kontext,
                'hCaptcha nicht erreichbar - die Eingabe wurde abgewiesen, nicht durchgelassen.'
            );
            return Captcha::WRONG;
        }
        ...
        if (array_intersect($codes, self::KONFIGURATIONSFEHLER) !== []) {
            ...
            Konfiguration::fehlerMerken('hCaptcha meldet ' . $liste . ' - bitte Sitekey und Secret prüfen.');
            PluginAudit::log(
                Plugin::SLUG,
     ...
     * curl-Erweiterung voraus, und ein Anbieter-Addon, das auf einer Instanz
...
class VerwaltungController extends BaseController {
    ...
    public function index(): void {
        ...
        PluginPage::render('hCaptcha', $inhalt);
    }
    ...
    public function speichern(): void {
        ...
        // welche (Framework#352).
        PluginAudit::log(
            Plugin::SLUG,
            'Zugangsdaten geändert',
            'Spam-Schutz hCaptcha',
            'Sitekey ' . ($siteRoh === '' ? 'entfernt' : 'gesetzt')
     ...
     * CSP-Freigabe das Widget LAUTLOS leer lässt. Bei hCaptcha ist es sogar
     * die halbe Freigabe, die trügt: Mit `js.hcaptcha.com` allein lädt das
     * Skript und der Rahmen bleibt leer - der Betreiber sieht ein Addon, das
     * "fast" geht, und sucht überall ausser in der Policy.
     *
    ...
    private function datenschutzKarte(): string {
        return '<div class="card">'
            . '<h1>🧩 hCaptcha</h1>'
            . '<p style="color: var(--text-muted);">hCaptcha ersetzt die eingebaute Rechenaufgabe des Kerns '
            ...
            . '<code>js.hcaptcha.com</code>. Dabei erhält <strong>Intuition Machines, Inc. (USA)</strong> '
            . 'seine IP-Adresse und technische Angaben seines Browsers.</li>'
            . '<li>hCaptcha bietet eine Auftragsverarbeitung mit EU-Bezug an. Das ordnet die Verarbeitung, '
            . 'es verhindert die Übermittlung nicht - Rechtsgrundlage (i. d. R. Art. 6 Abs. 1 lit. f DSGVO) '
            . 'und ein Eintrag <strong>in Ihrer Datenschutzerklärung</strong> bleiben nötig, mit Empfänger, '
            . 'Zweck und Drittlandbezug.</li>'
            . '<li>Ein Teil der Besucher bekommt eine <strong>Bildaufgabe</strong>. Wer sie nicht lösen kann, '
            . 'braucht den barrierefreien Weg von hCaptcha '
            . '(<code>accessibility.hcaptcha.com</code>) - der Hinweis darauf steht im Widget.</li>'
            ...
            . 'dieses Formular auf der eingebauten Rechenaufgabe. Wer ganz ohne Drittanbieter auskommen will, '
            . 'nimmt das Addon <code>captcha-altcha</code>.</li>'
            . '</ul>'
            . '</div>'
            . '</div>';
    }
    ...
    private function schluesselKarte(string $csrf): string {
        ...
        if (!Konfiguration::einsatzbereit()) {
            $html .= '<p style="color: var(--warning-fg, #8a6d3b); background: var(--surface-muted); '
                ...
                . 'Solange Sitekey <em>und</em> Secret fehlen, erscheint hCaptcha <strong>nicht</strong> in der '
        ...
        $html .= '<div class="form-group"><label for="hc-site">Sitekey (öffentlich, steht im HTML)</label>'
            . '<input class="form-control" type="text" id="hc-site" name="site_key" maxlength="120"'
            . ' value="' . htmlspecialchars($siteKey, ENT_QUOTES, 'UTF-8') . '" autocomplete="off">'
            . '<span class="form-hint">Aus dem hCaptcha-Dashboard. Leeres Feld = Schlüssel entfernen.</span></div>';

        $html .= '<div class="form-group"><label for="hc-secret">Secret (geheim)</label>'
            . '<input class="form-control" type="password" id="hc-secret" name="secret" maxlength="200"'
    ...
    private function cspKarte(string $csrf): string {
        ...
        $html .= '<p style="color: var(--text-muted);">Das Widget lädt Skript und Rahmen von fremden '
            . 'Herkünften. Stehen sie nicht <em>alle vier</em> in <code>captcha_domains</code>, blockiert der '
            . 'Browser sie <strong>ohne sichtbare Meldung</strong> - im Formular ist dann nichts oder nur ein '
            . 'leerer Rahmen zu sehen.</p>';
    ...
    private static function meldung(): string {
        $texte = [
            ...
            'site-ungueltig' => ['Der Sitekey hat kein gültiges Format (6-120 Zeichen, Buchstaben, Ziffern, - und _).', 'danger'],
        ...
        $status = is_string($_GET['hc'] ?? null) ? $_GET['hc'] : '';
    ...
    private function zurueck(string $status): never {
        header('Location: /plugin/' . Plugin::SLUG . '/verwaltung?hc=' . $status);
        exit;
    }
}
