...
Ansicht des Stammbaums führt (bis zu 8 Generationen, mehr als die 6 auf der
normalen Detailseite). Der Export erfolgt über die Browser-Druckfunktion
...
Keine Berechtigung nötig - die Export-Ansicht zeigt exakt dieselben Daten,
die bereits öffentlich auf `/hengst?id=...` sichtbar sind, nur anders
aufbereitet.
...
- `@page { size: A3 landscape; }` im Druck-Stylesheet sorgt dafür, dass auch
  tiefere Stammbäume (bis 8 Generationen) auf eine überschaubare Anzahl
  Seiten passen.
...
2. Optional Generationstiefe über `?depth=1-8` in der URL anpassen (Standard: 6).
